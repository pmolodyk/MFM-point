...
def visualize_pointcloud_batch(path, pointclouds, pred_labels, labels, categories, wandb_run=None, vis_label=False, target=None,  elev=30, azim=225):
    ...
    plt.savefig(path)
    if wandb_run:
        wandb_run.log({'generated_samples': wandb.Image(fig)})
    plt.close(fig)
